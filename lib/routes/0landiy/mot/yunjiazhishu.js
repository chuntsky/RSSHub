...
const config = {
    yanhaisanhuoyjzs: { title: '沿海散货运价指数', link: 'yunjiazhishu/yanhaisanhuoyjzs' },
    chukoujizhuangxiangyjzs: { title: '出口集装箱运价指数', link: 'yunjiazhishu/chukoujizhuangxiangyjzs' },
    ...
    const items = await Promise.all(
        list.map((item) =>
            ctx.cache.tryGet(item.link, async () => {
                ...
                item.description = content('div.ue_table').html();
